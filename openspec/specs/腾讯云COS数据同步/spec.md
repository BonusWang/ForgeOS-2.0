...
- **THEN** 系统使用当前命名空间下的 `forge-data.entities.v3.json` 作为 V3 主同步对象
...
系统 MUST 在云端不存在 V3 主同步对象时，使用当前浏览器端本地数据初始化 `forge-data.entities.v3.json`，并将该对象保存为本地共同基线。

#### Scenario: Browser initializes V3 after COS cleanup / 浏览器在 COS 清理后初始化 V3

- **WHEN** 浏览器端触发同步
- **AND** COS 前缀下不存在 `forge-data.entities.v3.json`
